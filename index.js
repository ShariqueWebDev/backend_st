...
app.get("/user/signin", async (req, res)=>{
  try {
    ...
    const response = await pool.query(`SELECT * FROM users WHERE email=$1 AND password=$2`, [email])
...
app.get("/api/users", async (req, res) => {
 ...
 return res.status(200).json({ message: "Welcome to user page", data: response.rows });
